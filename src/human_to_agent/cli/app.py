...
from human_to_agent.cli.output import emit
...
from human_to_agent.services.migrations import migration_status
...
def version(output_format: FormatOption = OutputFormat.text) -> None:
    """Show the Human to Agent version."""
...
def doctor(output_format: FormatOption = OutputFormat.text, root: RootOption = Path(".")) -> None:
    ...
    if not (resolved / "human-to-agent.yaml").is_file():
        emit(
            CommandResult(
                command="doctor",
                status="error",
