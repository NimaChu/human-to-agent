...
Run migration with `--dry-run` first. Migrations form a sequential version chain and each transform is pure: it receives a copy and must declare the next Schema version. Harness Foundry renders the candidate into an isolated tree and validates it before touching the source.
